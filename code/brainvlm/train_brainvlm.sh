...
MODE=${1:-full}
LOG=/pscratch/sd/s/sjmoon/FEELIN/output/brainvlm_ckpt/${MODE}/train.log
mkdir -p $(dirname $LOG)
...
case $MODE in
  smoke)
    OUT=/pscratch/sd/s/sjmoon/FEELIN/output/brainvlm_ckpt/fold1_VA_smoke
    ...
    ;;
  full)
    OUT=/pscratch/sd/s/sjmoon/FEELIN/output/brainvlm_ckpt/fold1_VA_full
    $PY code/brainvlm/train_brainvlm.py \
        --fold 1 \
        --epochs 3 \
        --skip_eval \
