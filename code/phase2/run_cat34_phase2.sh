...
#
# Usage:
#   bash code/phase2/run_cat34_phase2.sh                # all 16 cells sequentially
#   bash code/phase2/run_cat34_phase2.sh joint          # only joint 4 methods
#   bash code/phase2/run_cat34_phase2.sh brain_only     # only brain-only 4 methods
set -e
...
run_brain_only() {
  for task in "${TASKS[@]}"; do
    ...
    $PY_BRAIN code/phase2/brain_only/train_brain_supervised.py --task $task

    echo "===== Brain-only II_distillation / $task ====="
    $PY_BRAIN code/phase2/brain_only/train_brain_distillation.py --task $task

    echo "===== Brain-only III_multitask / $task ====="
    $PY_BRAIN code/phase2/brain_only/train_brain_multitask.py --task $task

    echo "===== Brain-only IV_subject_aware / $task ====="
    $PY_BRAIN code/phase2/brain_only/train_brain_subject_aware.py --task $task
  done
}
...
run_joint() {
  for task in "${TASKS[@]}"; do
    ...
    $PY_PHASE1 code/phase2/train_supervised.py --arch D --task $task

    echo "===== Joint A token_transformer / $task ====="
    $PY_PHASE1 code/phase2/train_supervised.py --arch A --task $task

    echo "===== Joint B cross_attention / $task ====="
    $PY_PHASE1 code/phase2/train_supervised.py --arch B --task $task
    ...
    if ! ls results/phase2/C/aligner_fold*_seed*.pt > /dev/null 2>&1; then
      ...
      $PY_PHASE1 code/phase2/train_contrastive.py
    else
      echo "===== Joint C Stage 1 aligners present, skipping training ====="
    fi
    echo "===== Joint C probe_brain_only / $task ====="
    $PY_PHASE1 code/phase2/probe_contrastive.py --task $task --probe_input brain_only
    echo "===== Joint C probe_joint / $task ====="
    $PY_PHASE1 code/phase2/probe_contrastive.py --task $task --probe_input joint
  done
}
