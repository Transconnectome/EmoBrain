...
def load_pretrained(model, ckpt_path: Path):
    ...
    # Handle pos_embed mismatch (10 time patches in ckpt vs 1 in our crop)
    key = "encoder.pos_embed_proj.emb_h"
    if key in new_state:
        ckpt_emb = new_state[key]
        model_emb = state_dict[key]
        if ckpt_emb.shape != model_emb.shape:
            n_rois = model_emb.shape[0]
            n_t_patches = ckpt_emb.shape[0] // n_rois
            if n_t_patches > 0 and ckpt_emb.shape[0] % n_rois == 0:
                reshaped = ckpt_emb.view(n_rois, n_t_patches, -1)
                new_state[key] = reshaped.mean(dim=1)
                print(f"  [pos_embed] {ckpt_emb.shape} → {new_state[key].shape} (averaged {n_t_patches} time patches)")
            else:
                del new_state[key]
