...
"왜 이 프로젝트가 필요한가", "왜 SwiFT-first인가", "왜 TRIBE v2와 naturalistic movie/story fMRI가
...
- HCP/CNeuroMod/StudyForrest-style naturalistic fMRI로 continued pretraining하는 것
...
## 4. 왜 naturalistic movie fMRI pretraining인가

여기서 핵심은 "movie-watching이 resting-state보다 당연히 낫다"가 아닙니다. 그 정도
주장은 너무 약합니다. NetFeeliX가 naturalistic movie fMRI를 쓰려는 이유는 더 구체적이어야
합니다.

```text
작은 emotion-labeled fMRI dataset으로 바로 emotion model을 학습하기 전에,
SwiFT가 stimulus-evoked brain dynamics를 먼저 학습해야 하는가?
```

Resting-state pretraining은 intrinsic connectivity, subject trait, baseline network structure를
잘 배울 수 있습니다. 반면 영화 시청 fMRI는 외부 stimulus에 의해 시간적으로 정렬된 brain
response를 제공합니다. Emotion task에서 중요한 정보는 얼굴, 장면, body motion, 목소리,
음악, 대사, narrative context, social cue, expectation이 시간 속에서 결합되는 방식입니다.
따라서 naturalistic movie fMRI는 "감정 라벨이 많아서" 쓰는 데이터가 아니라, emotion-labeled
dataset이 작을 때 SwiFT가 먼저 배워야 할 **stimulus-locked temporal representation**을 주는
데이터입니다.

EmoViS와도 연결됩니다. EmoViS가 vision feature가 emotion prediction에서 중요하다는 방향을
보여준다면, NetFeeliX의 movie fMRI pretraining은 그 다음 질문을 던집니다.

```text
visual/audio/language/social feature가 emotion target을 설명할 때,
그 feature에 의해 유도된 brain dynamics를 SwiFT가 더 잘 표현할 수 있는가?
```

이때 movie pretraining의 성공 기준은 "movie 데이터로 학습했더니 movie 관련 task에서
좋아졌다"가 아닙니다. 성공 기준은 Horikawa, Emo-FilM, Affective Videos 같은 emotion target으로
전이될 때, 특히 단순 arousal을 넘어 high-dimensional emotion vector나 appraisal/component
target에서 좋아지는 것입니다.

### 4.1 HCP는 첫 후보이지 유일한 답이 아니다

HCP Young Adult 7T movie는 scale, 표준화, 7T quality, resting/movie가 함께 있는 구조 때문에
첫 continued-pretraining 후보입니다. 하지만 HCP 하나만으로 naturalistic pretraining을
정당화할 수는 없습니다. 각 movie/story fMRI dataset은 서로 다른 가설을 테스트합니다.

| 데이터셋 | 왜 쓰는가 | NetFeeliX에서 확인할 질문 |
|---|---|---|
| HCP Young Adult 7T movie | subject 수와 표준화가 좋아 SwiFT continued pretraining의 첫 후보 | large-subject movie pretraining이 Horikawa/Emo-FilM transfer를 개선하는가 |
| CNeuroMod / Algonauts 2025 | video, audio, transcript, parcel response가 정리되어 TRIBE-style encoding에 적합 | stimulus-to-brain alignment가 emotion decoding에 도움이 되는가 |
| StudyForrest | 긴 영화 구조와 naturalistic audio/video response를 제공 | story continuity와 audiovisual narrative가 fMRI representation을 어떻게 바꾸는가 |
| Narratives | audio story와 transcript 중심의 naturalistic language fMRI | visual cue 없이 language/narrative context만으로 affective context를 정렬할 수 있는가 |
| 101 Dalmatians | audiovisual, auditory, visual condition 비교에 유리 | vision-only, audio-only, audiovisual feature 중 무엇이 emotion transfer에 중요한가 |
| Emo-FilM / REELMO | emotion/component/trajectory target이 있음 | pretraining source가 아니라 downstream/target validation에 가깝다 |

즉 "HCP movie를 쓰자"가 아니라,

```text
naturalistic fMRI source를 model hypothesis별로 선택하자
```

가 정확한 전략입니다.

### 4.2 Movie pretraining이 잘못 배울 수 있는 것

Movie fMRI pretraining은 강력하지만 위험합니다. 모델이 emotion representation을 배운 것이
아니라 다음 shortcut을 배울 수 있습니다.

- low-level visual motion, luminance, scene cut
- auditory energy, speech onset, music intensity
- face/object category
- stimulus identity
- subject synchronization 자체
- arousal만 잘 잡는 generic attention signal

그래서 NetFeeliX는 movie pretraining을 할 때 반드시 통제 실험을 같이 둬야 합니다.

| 검증 | 의미 |
|---|---|
| resting-SwiFT vs movie-SwiFT | movie-evoked dynamics가 실제로 전이에 도움이 되는지 |
| low-level feature control | motion/luminance/audio-energy shortcut인지 |
| vision-only/audio-only/text-only ablation | emotion transfer에 어떤 modality가 필요한지 |
| stimulus-only vs brain-only | label이 stimulus만으로 설명되는지 |
| Horikawa vs Emo-FilM transfer | short video affect geometry와 naturalistic component target에 모두 통하는지 |
| arousal-only vs high-dimensional target | 단순 arousal 개선인지 rich emotion representation 개선인지 |

이렇게 해야 movie pretraining이 "그럴듯한 대형 실험"이 아니라 모델 선택을 위한 증거가 됩니다.

### 4.3 결론: pretraining source가 아니라 가설 검증 장치다

HCP movie pretraining을 쓰는 이유는 HCP가 유명해서가 아닙니다. 이유는 다음입니다.

```text
SwiFT가 resting-state representation에서 naturalistic stimulus-locked representation으로
이동할 때, emotion downstream transfer가 실제로 좋아지는지 검증하기 위해서다.
```

만약 좋아지면 NetFeeliX는 naturalistic fMRI continued pretraining을 확장합니다. HCP 이후에는
CNeuroMod/Algonauts, StudyForrest, modality-control dataset을 붙일 수 있습니다.

만약 좋아지지 않으면 결론도 중요합니다. 그 경우 emotion-specific head, subject adapter,
TRIBE-style alignment, target 재설계가 더 중요한 방향일 수 있습니다.
...
### Naturalistic movie/story fMRI

HCP 7T movie는 emotion label이 없는 첫 pretraining 후보입니다. 하지만 자연주의적
pretraining의 전체 전략은 HCP 하나가 아닙니다.

역할은:

```text
SwiFT가 stimulus-locked visual/audio/language/social/narrative brain dynamics를
학습할 때 emotion transfer가 좋아지는지 검증하는 것
```

입니다.

HCP는 large-subject continued pretraining, CNeuroMod/Algonauts는 multimodal
alignment, StudyForrest는 long-film continuity, Narratives는 language/story context, 101
Dalmatians는 modality-control 질문에 맞게 사용합니다.
...
5. 다음 투자는 SwiFT adaptation인가, naturalistic pretraining인가, TRIBE-SwiFT alignment인가?
...
### 4단계: naturalistic movie/story continued pretraining
...
HCP는 첫 후보지만, 다른 naturalistic dataset은 alignment, long-context, language-context,
modality-control 같은 구체적인 질문이 있을 때 붙입니다.
...
3. naturalistic movie/story pretraining은 실제로 도움이 되는가?
...
> emotion-labeled fMRI, naturalistic movie/story fMRI pretraining, TRIBE v2-style
...
HCP를 첫 후보로 naturalistic movie/story fMRI pretraining의 전이 효과를 확인한다.
