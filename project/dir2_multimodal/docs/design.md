...
| Source | Shape | 위치 |
|--------|-------|------|
| Brain (Brain-JEPA resting frozen) | (5 subj, 2185 stim, 768) | `project/shared/output/embeddings/brain_jepa_resting_pad-zero/sub-XX.pt` |
...
| Phase 1 Brain-JEPA frozen (raw) | 0.330 | 0.221 | 0.669 |
| Phase 1 V-JEPA2 pretrained (raw, no align) | (TODO 추출) | (TODO) | (TODO) |
| **본 학습. Brain-only (after align)** | ? | ? | ? |
| **본 학습. Video-only (after align)** | ? | ? | ? |
| **본 학습. Joint** | ? | ? | ? |

**핵심 KPI**. $\Delta_{\text{brain unique}}$ 가 paired bootstrap p < 0.05 + 절대값 +0.05 이상.
...
| 2 | Fold 1 pilot 학습 (SigLIP + GRL, 40 epoch) + variance partition | `pilot_metrics.csv` 의 (B, V, J) 3-way |
| 3 | 표 + visualization (UMAP, similarity matrix) + Phase 1 비교 | `figures/` 안의 plot, slide 1 page summary |
