# (2024) Neural Embeddings Rank_Aligning 3D latent dynamics with movements

**Source:** (2024) Neural Embeddings Rank_Aligning 3D latent dynamics with movements.pdf

---

## Page 1

Neural Embeddings Rank: Aligning 3D latent
dynamics with movements
Chenggang Chen, Zhiyu Yang, Xiaoqin Wang
Department of Biomedical Engineering, Johns Hopkins University
cheng-gang.chen@jhu.edu
Abstract
Aligning neural dynamics with movements is a fundamental goal in neuroscience
...
First, visualizing neural dynamics. This involves a trade-off between dimensionality and explained
...
However, these trajectories do not align precisely with the reaching movements: when the hand
...
As our goal is to extract latent dynamics that are most informative about movements, we chose to
...
We are motivated by the fact that CEBRA treats continuous labels as many discrete classes, which
...
We introduce Neural Embeddings Rank (NER), a dimensionality reduction method that contrasts
...
We demonstrate that NER reveals behavior-aligned latent dynamics in all twenty sessions from three
...
200
-200
Figure 1: a Top: Hand velocities of X-Y coordinates across three trials. Note that the distribution of
...
3
CEBRA & piVAE
Figure 2: NER aligns 3D latent dynamics with movements and enabling cross sessions movement
...
2
Related work
There are at least five categories of dimensionality reduction methods: Linear methods, such as PCA,
...
such as CEBRA [27] and Mine Your Own view (MYOW) [1]. Compared to piVAE, AutoLFADS,
...
3
Model
NER is inspired by previous studies [31] and uses the same data sampling and neural feature encoder
...
Mathematically, we define x as the high-dimensional neural dynamics, f as the feature encoder,
...
In each iteration, CEBRA receives one batch of anchors vi, one batch of embeddings vj that will
...
In each iteration, NER receives one batch of anchors vi, one batch of augmented embeddings vj, vk
...
There are two key differences from CEBRA. First, batches of vi and vj, vk are merged, and labels
...
By ranking all the embeddings within a batch, NER also addresses the class imbalance issue and
...
4
Results
To fairly evaluate our dimensionality reduction method against related approaches, we selected six
...
piVAE, and CEBRA. To avoid bias from a single session in a specific brain area, where piVAE and
...
Across all ten sessions in the left and right hemispheres of M1, NER consistently revealed neural
...
Figure 3: NER reveals consistent, movement-aligned latent dynamics in M1. a Single-trial (top) and
...
revealed by dPCA.
CEBRA was the second-best method, uncovering comparable latent dynamics with both directions
...
differing from ground truth movements, and its latent dynamics were less consistent across sessions.
For example, it only revealed connected latent dynamics at movement starting points in two sessions.
piVAE ranked third, displaying direction-aligned latent dynamics in different directions with relatively
...
In summary, NER proved to be the best method for revealing movement-aligned latent dynamics. We
...
Five dimensionality reduction methods were used to reveal single-trial latent dynamics that depended
...
Fig. 4a shows the ground truth and predicted hand movement trajectories using latent dynamics
revealed by NER in PMd. A linear regression decoder explained 90% and 98% of the variance in
hand velocities and positions, respectively.
a
b
c
d
NER
Directions and X-Y velocities
Velocities
Positions
Hand movements
Explained movements
...
S1
S1
Figure 4: Explained variance of movements in M1, PMd, and S1. a Left: Hand movement trajectories.
...
In both M1 and PMd (Fig.4b, Fig.13a), a logistic regression decoder revealed the tuning of directional
...
correlated with hand velocities in the latent dynamics extracted by NER but not by CEBRA (M1:
0.93 vs. 0.28, PMd: 0.93 vs. 0.13; Fig. 13b).
NER outperformed the four other methods in all sessions for explaining the variance of both hand
...
In summary, combined with linear decoders, NER demonstrated the clearest velocity-dependent
...
NER explains the largest variance in both hand velocities and positions in the 80% training data.
...
0
1 or 100%
Figure 5: Decoding within and across time and brain hemispheres over years in M1. a-d Three
...
Fig.5a shows velocity decoding performance using a linear regression decoder across three dimen-
...
NER outperformed CEBRA and piVAE by a substantial margin in position decoding across all
...
In summary, compared to CEBRA and piVAE, NER consistently achieved higher performance across
...
Next, we turned our attention to PMd. Surprisingly, NER revealed similar latent dynamics in
...
We also evaluated position and direction decoding and found that NER continued to outperform the
...
0
1
Figure 6: Latent dynamics in PMd and decoding across brain areas. a Trial-averaged latent dynamics
...
Lastly, we examined the latent dynamics and movement decoding in S1. NER revealed consistent
...
Explained
Decoded
Figure 7: Distinct latent dynamics with the same movement in S1. a Trial-averaged latent dynamics
...
Lastly, we applied both NER and CEBRA in a new experiment where a monkey performed both
...
We first examined the latent dynamics when the monkey performed straight hand movements in
...
than CEBRA (Fig.17f).
Overall, NER not only aligns latent dynamics with straight movements but also effectively differ-
entiates curved hand movements from straight ones, demonstrating its robustness across various
movement types.
5
Discussion
A benchmark comparison of NER and six other dimensionality reduction methods across multiple
...
those uncovered by PCA. We believe NER will aid neuroscientists in probing the stability of latent
9
...
curve-curve 
move 
Figure 8: Distinct latent dynamics align with different movements in M1. a A monkey performs a
...
dynamics under various conditions. For BMI applications, we demonstrate that NER, combined with
...
In our final experiments with curved movements, we manually selected three pairs of reaching tasks
...
We greatly appreciate the Miller and Shenoy labs for publicly releasing their experimental data on
...
References
[1] Mehdi Azabou, Mohammad Gheshlaghi Azar, Ran Liu, Chi-Heng Lin, Erik C Johnson, Kiran
...
[2] Matteo Carandini and David J Heeger. Normalization as a canonical neural computation. Nature
reviews neuroscience, 13(1):51–62, 2012.
[3] Nicholas S Card, Maitreyee Wairagkar, Carrina Iacobacci, Xianda Hou, Tyler Singer-Clark,
...
391(7):609–618, 2024.
[4] Le Chang and Doris Y Tsao. The code for facial identity in the primate brain. Cell, 169(6):1013–
1028, 2017.
[5] Mark M Churchland, John P Cunningham, Matthew T Kaufman, Justin D Foster, Paul Nuyu-
jukian, Stephen I Ryu, and Krishna V Shenoy. Neural population dynamics during reaching.
Nature, 487(7405):51–56, 2012.
[6] Chaofei Fan, Nick Hahn, Foram Kamdar, Donald Avansino, Guy Wilson, Leigh Hochberg, Kr-
...
[7] Cátia Fortunato, Jorge Bennasar-Vázquez, Junchol Park, Joanna C Chang, Lee E Miller,
...
[8] Winrich A Freiwald, Doris Y Tsao, and Margaret S Livingstone. A face feature space in the
macaque temporal lobe. Nature neuroscience, 12(9):1187, 2009.
[9] Juan A Gallego, Matthew G Perich, Raeed H Chowdhury, Sara A Solla, and Lee E Miller.
Long-term stability of cortical population dynamics underlying consistent behavior. Nature
neuroscience, 23(2):260–270, 2020.
[10] Juan A Gallego, Matthew G Perich, Lee E Miller, and Sara A Solla. Neural manifolds for the
control of movement. Neuron, 94(5):978–984, 2017.
[11] Juan A Gallego, Matthew G Perich, Stephanie N Naufel, Christian Ethier, Sara A Solla, and
...
[12] Cecilia Gallego-Carracedo, Matthew G Perich, Raeed H Chowdhury, Lee E Miller, and Juan Ál-
...
[13] Joshua I Glaser, Ari S Benjamin, Raeed H Chowdhury, Matthew G Perich, Lee E Miller, and
Konrad P Kording. Machine learning for neural decoding. Eneuro, 7(4), 2020.
[14] Cole Hurwitz, Akash Srivastava, Kai Xu, Justin Jude, Matthew Perich, Lee Miller, and Matthias
Hennig. Targeted neural dynamical modeling. Advances in Neural Information Processing
Systems, 34:29379–29392, 2021.
[15] Mohammad Reza Keshtkaran, Andrew R Sedler, Raeed H Chowdhury, Raghav Tandon, Diya
...
[16] Dmitry Kobak, Wieland Brendel, Christos Constantinidis, Claudia E Feierstein, Adam Kepecs,
...
[17] Eric Kenji Lee, Hymavathy Balasubramanian, Alexandra Tsolias, Stephanie Udochukwu
...
[18] Ran Liu, Mehdi Azabou, Max Dabagia, Chi-Heng Lin, Mohammad Gheshlaghi Azar, Keith
...
[20] Sean L Metzger, Kaylo T Littlejohn, Alexander B Silva, David A Moses, Margaret P Seaton,
...
[22] Felix Pei, Joel Ye, David Zoltowski, Anqi Wu, Raeed H Chowdhury, Hansem Sohn, Joseph E
...
[23] Matthew G Perich, Juan A Gallego, and Lee E Miller. A neural population mechanism for rapid
learning. Neuron, 100(4):964–976, 2018.
[24] Neil C Rabinowitz, Ben DB Willmore, Jan WH Schnupp, and Andrew J King. Contrast gain
control in auditory cortex. Neuron, 70(6):1178–1191, 2011.
[25] Mostafa Safaie, Joanna C Chang, Junchol Park, Lee E Miller, Joshua T Dudman, Matthew G
...
[28] Laurens Van der Maaten and Geoffrey Hinton. Visualizing data using t-sne. Journal of machine
learning research, 9(11), 2008.
[29] Francis R Willett, Donald T Avansino, Leigh R Hochberg, Jaimie M Henderson, and Kr-
ishna V Shenoy. High-performance brain-to-text communication via handwriting. Nature,
593(7858):249–254, 2021.
[30] Joel Ye and Chethan Pandarinath. Representation learning for neural population activity with
neural data transformers. arXiv preprint arXiv:2108.01210, 2021.
[31] Kaiwen Zha, Peng Cao, Jeany Son, Yuzhe Yang, and Dina Katabi. Rank-n-contrast: Learning
continuous representations for regression. Advances in Neural Information Processing Systems,
36, 2024.
[32] Ding Zhou and Xue-Xin Wei. Learning identifiable and interpretable latent models of high-
...
[33] Feng Zhu, Harrison A Grier, Raghav Tandon, Changjia Cai, Anjali Agarwal, Andrea Gio-
...
A.1
Computer
Operating System is Ubuntu 22.04.3 LTS, computer memory is 42 GB, CPU is Intel Xeon W-2225,
and GPU is NVIDIA RTX A5000.
A.2
Center-out reaching experiments in M1, PMd, and S1
A.2.1
Behavior
The dataset comprises behavioral task data from two male Macaca mulatta monkeys (Monkeys H and
...
Monkey C: Trained to wait for an auditory go cue during a delay period of 0.5–1.5 seconds while
...
Monkey H: No delay period; the monkey had to move the cursor to the outer target within 1 second
and hold it there for 0.1 seconds.
For both monkeys, the trial restarted by returning the cursor to the central target. Endpoint positions
...
session, mean ± s.d.)
Neural recordings
The dataset consists of neural recordings from two male Macaque monkeys. These recordings were
obtained using 96-channel Utah electrode arrays implanted in specific cortical regions.
Monkey C: Initially implanted in the right primary motor cortex (M1) and later received implants in
...
Neural activity was recorded using a Cerebus system (Blackrock Microsystems, Salt Lake City, UT)
...
Datasets
All the center-out reaching experiments using the open source data from: https://datadryad.
...
The MC_Maze dataset includes recordings from the primary motor and dorsal premotor cortices of a
...
A.3
Decoders
We rotate the latent dynamics with reference to same target before decoding. We used orthogo-
...
M1 in Left Hemisphere
M1 in Right Hemisphere
Figure 9: NER reveals consistent and movements aligned latent dynamics in M1 for the remaining
four sessions. Extra four sessions’ latent dynamics at left and right hemisphere of Monkey C after
rotating relative to target session in Fig 3 (2016-10-14).
16
...
06-29
06-30
Figure 10: Neural embeddings revealed by five other dimensionality reduction methods. Single trial
...
w/
label
Figure 11: Latent dynamics without rotation. Trial-averaged latent dynamics revealed by five
...
Monkey C 2016-10-05
Monkey C 2016-09-29
Figure 12: Mixed single trial latent dynamics in PCA and time-stimulus components revealed by
...
Table 2: Statistical analysis and quantitative comparisons of the presented results. Figs. 3 and 6a
...
PMd & S1
c
Explained directions in PMd and X-Y velocities
Explained peak classification accuracy by logistic regression
Figure 13: Hand directions tunings and explained peak classification accuracy of hand directions in
...
piVAE
b
Figure 14: Latent dynamics in PMd revealed by CEBRA and piVAE. Single trial and trial averaged
...
0
100%
Figure 15: Direction decoding accuracy. Same date, cross date, and cross brain areas decoding of
hand directions using linear regression (left) and k-nearest neighbors decoder trained on the latent
dynamics revealed by three methods. Notice the range of color bars is 0-100 for all six figures.
20
...
12-07
c
Figure 16: Latent dynamics in S1 revealed by CEBRA and piVAE. a Top, single trial latent dynamics
...
dimensionality reduction method. c Similar to c but using piVAE dimensionality reduction method.
21
...
f
e
Figure 17: Distinct latent dynamics revealed by CEBRA. Similar to Fig 7 but using the latent
...
the experiments?
Answer: [Yes]
Justification: We provide the details of our computer that used to run those experiments.
Guidelines:
• The answer NA means that the paper does not include experiments.
...
Answer: [Yes]
Justification: We used open-source neural spiking data from animals.
Guidelines:
...
Answer: [NA]
Justification: The data we used came from neuron activities in the brains. Nothing is related
to humans.
Guidelines:
...
Answer: [NA]
Justification: The codes have been released by others already.
Guidelines:
...
properly respected?
Answer: [NA]
Justification: All the data and codes are open source
Guidelines:
...
Answer: [NA]
Justification: Everything is open source already.
Guidelines:
...
Answer: [NA]
Justification: Only authors do the experiments.
Guidelines:
...
Answer: [NA]
Justification: We used open source data.
Guidelines:
• The answer NA means that the paper does not involve crowdsourcing nor research with
human subjects.
