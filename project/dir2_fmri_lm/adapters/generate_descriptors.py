...
Emotion task 의 5 desc_type 후보 (fMRI-LM 의 fc/ica/gradient/graph 대신).
    - va_descriptor       valence/arousal 값을 자연어로 (continuous regression task)
    - va_binary           valence/arousal 의 Q1 vs Q4 binary label 자연어로 (binary classification task)
...
VA_BINARY_TEMPLATE = "This brain response shows {v_label} valence and {a_label} arousal."
...
def make_va_binary(va_df: pd.DataFrame, v_q_col: str, a_q_col: str, sample_col: str) -> pd.DataFrame:
    """Q1 -> 'low', Q4 -> 'high', OTHER -> skipped (row not written)."""
    rows = []
    label_map = {"Q1": "low", "Q4": "high"}
    for _, r in va_df.iterrows():
        v_q, a_q = r[v_q_col], r[a_q_col]
        if v_q not in label_map and a_q not in label_map:
            continue
        v_lbl = label_map.get(v_q, "moderate")
        a_lbl = label_map.get(a_q, "moderate")
        rows.append({"sample_id": r[sample_col],
                     "text": VA_BINARY_TEMPLATE.format(v_label=v_lbl, a_label=a_lbl)})
...
def main() -> None:
    ...
    p.add_argument("--va-csv", help="rows of (sample_id, valence, arousal, [valence_quartile, arousal_quartile])")
    ...
    p.add_argument("--v-quartile-col", default="valence_quartile")
    p.add_argument("--a-quartile-col", default="arousal_quartile")
    ...
    if args.va_csv:
        ...
        if args.v_quartile_col in va_df.columns and args.a_quartile_col in va_df.columns:
            df_bin = make_va_binary(va_df, args.v_quartile_col, args.a_quartile_col, args.sample_col)
            df_bin.to_csv(out / "va_binary_descriptors.csv", index=False)
            print(f"[desc] wrote {len(df_bin)} va_binary descriptors (Q1+Q4 only)")
        else:
            print("[desc] skipping va_binary (quartile columns missing — run va_quartile_split.py first)")
