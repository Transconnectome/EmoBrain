...
(2026-06-29 pivot 반영. 이전 direction-split framing (D1 BrainVLM / D2 fMRI-LM / D3 CCN) 폐기. Single project + framework novelty path 의 5 NV spine. 이전 version 은 `archive/v5_direction_split_20260628/Paper/framework_EN.md` 에 보존.)

## Title

**EmoBrain. Decoding fine-grained emotion from human brain activity.**

(Title is locked. Sub-title 의 detail working draft 은 별도. 본 title 이 paper / talk / repo 모두 의 공식 name.)

---

## 한 줄 요지 (One-line summary)

A single LLM-based foundation model decodes fine-grained emotion (V/A 점수, Cat34 top-1 / top-2 / top-k / 34D distribution) from human brain activity, by fusing brain tokens + naturalistic video tokens + human-written neutral caption tokens in one forward pass, with a modular brain encoder backbone (raw ROI / Ridge / BFM / VLM) that is swappable for fair architectural ablation.

이전 frozen BFM single-modality decoder 의 ceiling 한계 (Phase 1) + token-output VLM 의 numeric mapping 한계 (D1 v1/v2 negative result) 를 evidence 로, *왜 single-modality 만 으로 fine-grained emotion 의 decoding 이 안 되는지* 의 boundary 를 정의 한 후, 3-modality fusion + 4-stage curriculum + modular encoder ablation 으로 그 boundary 를 *넘는 가* 의 first systematic test.

---

## Spine question

**The neuroscientific question the foundation model is built to answer.**

Fine-grained emotion (34 Cowen-Keltner category 의 distribution + V/A continuous) 의 representation 이 human brain 에서 *얼마나 decodable* 한가. 그리고 그 decoding 이 *brain activity unique* 한 contribution 인가 (= 같은 stimulus 의 video 의 visual content 와 human caption 의 semantic content 를 *control* 한 후 의 brain residual contribution).

이 question 은 model design 의 question 이 아니다. Foundation model = *answering instrument*. Human label agreement 의 reference point = Cowen-Keltner 2017 의 categorical concordance (평균 54%, chance 27%; 이는 ICC 가 아니라 같은 emotion category 를 고른 rater 비율). Brain unique contribution 자체 = modality variance partitioning 의 unique-brain term. 둘 다 *measure-able neuroscientific quantity*. (주의. concordance 54% 는 categorical yes/no 일치율 이라 우리 34D continuous 회귀 metric 의 ceiling 으로 직접 환산 불가. §Cowen concordance note 참조.)

Model 의 design choice (3-modality fusion, modular encoder, 4-stage curriculum) 는 이 question 의 *resolution + denoising* instrument. Question 자체 와 conflate 하지 않는다.

### 4 sub-question (anchored to spine)

- **(a) Affect decoding ceiling.** Brain decoding 이 얼마까지 도달 가능 한가. 참고. Cowen-Keltner categorical concordance 는 평균 54% (chance 27%) 이지만 이는 categorical 일치율 이라 우리 continuous metric 의 직접 ceiling 아님. 진짜 ceiling 은 별도 방법 (brain cross-subject ISC, 또는 label 자체 의 crowd split-half) 으로 측정. 도달 못 한다 면 그 *gap* 의 nature 는 무엇인가 (noise vs irreducible individual difference vs missing modality).
- **(b) Brain-unique vs shared contribution.** Brain only 의 decoding accuracy 와 brain + video + caption joint 의 accuracy 의 *delta* 가 video / caption 의 dimension 의 어느 부분 에 의해 explain 되는가.
...
- **(d) Behavioral-brain dissociation.** Semantic decoding (Tang/Huth 2023, Horikawa 2024) 은 brain 에서 well-decoded. Affect decoding 은 ceiling-bound. 같은 architecture 에서 두 task 의 *dissociation* 을 reproduce 할 수 있는가. 이는 brain affect code 의 *structural difference* 의 evidence.
...
Emotion fMRI 분야 에서 LLM (large language model) 을 *fine-grained emotion decoding instrument* 로 통합 한 first paper. Prior work (Bush 2018 brain states, Saarimäki 2018 cat-specific patterns, Horikawa 2020 27-cat decoding) 은 모두 classical decoder (ridge / SVM / shallow MLP). EmoMind (Mohammed 2026) 은 LLM 사용 하지만 emotion decoding 의 evaluation 이 caption-rewriting 의 단일 task. EmoBrain 은 V/A + Cat34 의 multi-task fine-grained decoding 의 LLM-first instrument.

### NV1. 3-modality LLM fusion

Brain + video + caption 의 *3 modality 모두 의 LLM token sequence 통합 fusion* 의 first emotion fMRI paper. Prior multi-modal brain encoding (TRIBE Algonauts 2025, Singh 2025 multi-modal) 은 transformer fusion 의 표준 framework 이지만 emotion 의 evaluation 안 함. UMBRAE (ECCV 2024), MindLLM (2025) 의 brain + text fusion 은 *brain-to-text generation* 이지 emotion decoding 아님. 3-modality + emotion = unique combination.

### NV2. MindCaptioning human-written neutral caption as bridge

Horikawa 2024 (Science Advances DOI 10.1126/sciadv.adw1464) 의 MindCaptioning 의 human-written neutral caption (per video, crowd-sourced, affect-neutral content description) 을 brain-context bridge 로 활용. Brain token 의 *semantic anchor* 의 역할. 우리 의 자체 model-generated caption (frozen Qwen3-VL 의 video → text) 도 *parallel comparison* 으로 추가, human vs model caption 의 affect-decoding 의 contribution 분석.
...
학습 = LoRA + projector + brain encoder (variant 에 따라 trainable or frozen). LLM body = frozen.

---

## Modular brain encoder ablation

4 brain encoder variants. 같은 fusion + 같은 head + 같은 evaluation 에서 fair comparison.
...
MindCaptioning (Horikawa 2024, Science Advances) 은 Horikawa 2020 의 *같은 2185 video 의 superset* (2196 video) 에 대해 *human crowd-source* 의 *affect-neutral content description* 을 수집 한 dataset. "A woman is walking on a beach at sunset" 처럼 *what happens* 의 description, *how it feels* 의 description 아님.
...
- *Affect-neutral by construction*. Caption 자체 에 affect label leak 없음. Fusion 의 affect prediction 이 *caption alone* 으로 가능 하지 않음 의 guarantee. → modality variance partitioning 의 *unique-brain* term 의 정량 가능.
...
### Our model-generated captions (comparison)

Frozen Qwen3-VL (no fine-tune) 의 video → caption pipeline 으로 *same 2185 video* 의 model caption 생성. Human caption 과 *parallel input* 으로 fusion.
...
- **Interpretation**. "이 영상 은 기쁨 과 향수 가 높고 공포 는 낮다" 의 윤곽 (Pearson) 을 맞히되, 그 강도 (CCC) 까지 맞히는 것 이 목표.
...
각 tier 의 result 의 *gap* 자체 가 finding 의 component. Tier 5 - Tier 2 = *novelty 의 lift*, Tier C - Tier 5 = *remaining gap to ceiling*. (주의. Cowen concordance 54% 는 categorical 일치율 이라 여기 Tier C 로 직접 못 씀. 실제 ceiling 은 brain ISC 또는 label split-half 로 측정.)

### Modality variance partitioning

4 condition. Brain only / Video only / Caption only / Joint.
...
SC2 (brain-unique contribution) 의 직접 instrument. Unique-brain 의 정량 = brain 의 *information beyond stimulus content* 의 quantity.

### Cowen-Keltner concordance note (2026-07-07 정정)

**정확한 사실 (원문 검증).** Cowen-Keltner 2017 PNAS 원문 (PMC5617253). "75% of the videos elicited significant concordance for at least one category of emotion across raters (FDR < 0.05), with concordance averaging 54% (chance level being 27%)". 즉.
- **ICC 가 아님.** Concordance = 한 영상 에 대해 같은 emotion category 를 고른 rater 의 비율 (평균 54%, chance 27%).
...
**Ceiling 으로 직접 못 쓰는 이유.** Concordance 54% 는 categorical 일치율. 우리 headline 은 continuous 34D profile Pearson. 단위 가 달라 "ridge 0.29 = ceiling 0.54 의 54%" 같은 환산 은 통계적 으로 부적절. 이전 문서 의 "ICC 0.54 를 fraction 으로 normalize" 계획 은 철회.

**진짜 noise ceiling.** 별도 방법 으로 측정.
- Brain cross-subject ISC (5 subject 가 같은 자극, 우리 데이터 로 측정 가능).
- Label 자체 의 crowd split-half (rater 를 반 으로 나눠 34D proportion 의 재현성). 원 rater-level 데이터 확보 시 가능.
- Concordance 54% 는 문맥 참고 값 으로만 인용.

### Behavioral-brain dissociation

Semantic decoding (Tang & Huth 2023 Nature Neuroscience DOI 10.1038/s41593-023-01304-9, Horikawa 2024 Science Advances DOI 10.1126/sciadv.adw1464) 은 brain 에서 well-decoded. Pearson r 0.5+, language-like generation quality high.

Affect decoding 은 *동일 architecture* 에서 도 ceiling-bound (Phase 1 의 0.42 r 의 V/A 등). 두 task 의 dissociation 을 *우리 framework 에서 reproduce* = brain affect code 의 *structural difference* 의 evidence.
...
5 subject pooled training 의 fold structure 를 *4 subject train + 1 subject zero-shot* 의 5 fold 로 변경. 4 fold 학습 후 5th subject 의 brain 만 fusion 에 attach (re-training 없음, ICL only or projector zero-shot). Cross-subject generalization 의 정량.
...
Null result interpretation. 3-modality fusion 의 lift 가 없 다 면 = *brain modality 의 information 이 video + caption 에 의해 fully captured* = "brain decoding 의 unique contribution 없음" 의 boundary 정의. *Publishable negative result*. EmoMind 의 "brain better than label" 의 claim 의 partial refutation.

### SC2. Brain-unique R²

Modality variance partitioning 의 *unique-brain* R² 가 *unique-video* R² 와 *unique-caption* R² 의 *합* 보다 적어도 한 task (V binary, V regression, Cat34 top-1, 34D KL 중) 에서 유의 하게 큼. Threshold. unique-brain R² ≥ 0.05 (absolute), bootstrap 95% CI 가 zero 안 포함.

Null result interpretation. unique-brain 이 video / caption 보다 작 다 면 = "brain 이 stimulus content 의 redundant carrier 일 뿐" 의 finding. Affect 의 brain representation 의 *uniqueness* 의 부재. fMRI affect 분야 의 boundary marker.

### SC3. Modular encoder ranking generalization

E1 raw ROI / E2 ridge / E3 BFM / E4 VLM 의 ranking 이 Stage 1-4 의 4 task 에서 *consistent*. Phase 1 의 E2 > E3 의 ranking 이 multi-modal fusion + curriculum 에서 도 hold 하는지. Threshold. Spearman rank correlation 0.7+ across stage.

Null result interpretation. Ranking inconsistency = encoder choice 가 *task-dependent* = "encoder universality 없음" 의 finding. Modular encoder design 의 *necessity* 의 evidence (한 encoder choice 가 all task 에 best 아님).
...
Null result interpretation. LOSO fail = subject-pooled fusion 의 *new-subject generalization 의 한계*. Cross-subject universal code 의 부재 의 evidence. Per-subject finetune 의 *necessity* 의 boundary.

### SC5. Behavioral-brain dissociation reproduction
...
본 pivot 의 시점 의 *preserved evidence base*. 5 NV framework 의 Section 4 (modular encoder ablation) 의 evidence 로 re-frame. 결과 *날리지 않음*.
...
진단. Token output formulation 의 fundamental limit. 4 backbone size 모두 plateau = capacity issue 아님. Section 4 의 *E4 token-output variant 의 negative result*. Backbone hidden state 직접 사용 (current REG variant + new EmoBrain fusion) 의 motivation evidence.
...
MindCaptioning 은 *semantic decoding* 의 strong reference. Brain → caption 의 generation 의 quality 가 *language-level coherence* 와 *semantic accuracy* 모두 strong. 11 subject × 2196 video 의 large cohort.

**우리 의 활용.**
- *Human-written caption 의 source*. 우리 의 NV2 의 caption input 의 origin.
...
Competitor 가 아닌 *complementary*. Our paper 의 *semantic-affect dissociation* 의 evidence base.
...
- **MindCaptioning (Horikawa 2024) (cross-cohort stretch + human caption source).** 11 subj × 2196 video. Cowen-Keltner 2196 와 100 % stim overlap. Human-written caption per video. Science Advances DOI 10.1126/sciadv.adw1464.
...
- Saarimäki, H., et al. (2018). Discrete neural signatures of basic emotions. *Cerebral Cortex*. (Cat-specific MVPA reference.)

---

## Open decisions

- **OD1. POYO ablation timing.** Qwen3-VL 의 LLM body 의 *alternative ablation* 으로 POYO (neural sequence model) 통합 시점. EmoBrain main result 후 supplementary 인가, 처음부터 main ablation 인가.
...
- **OD5. Video encoder which one.** CLIP / V-JEPA2 / VideoMAE 중 main + ablation 의 choice. V-JEPA2 는 SOTA 지만 compute heavy.
