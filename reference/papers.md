...
- **FEELIN role**: direct fMRI backbone baseline.
...
- **FEELIN role**: bridge baseline for resting-state to emotion-related task reactivity.
...
- **FEELIN role**: pretrained BFM transfer baseline.
...
- **FEELIN role**: strong methodological precedent for JEPA-style naturalistic fMRI pretraining.
...
- **FEELIN role**: state-of-the-art BFM baseline if code/weights are available.
...
- **FEELIN role**: useful reference for atlas-free direction, likely not first two-month implementation unless code is easy.
...
- **FEELIN role**: future-method reference, especially for multi-state pretraining.
...
- **FEELIN role**: compare against BFM; inspiration for stimulus-brain alignment.
...
- **FEELIN role**: newest stimulus-brain alignment reference.
...
- **FEELIN role**: video feature extractor for stimulus-brain-emotion alignment.
...
- **FEELIN role**: practical architecture reference for stimulus feature fusion and temporal prediction in movie fMRI.
...
- **FEELIN role**: supports the claim that dynamic multimodal integration should help stimulus-brain-emotion alignment.
...
- **FEELIN role**: high-dimensional emotion benchmark if data access is feasible.
...
- **FEELIN role**: modern downstream dataset for naturalistic emotion prediction.
...
- **FEELIN role**: baseline method and hypothesis support that arousal may be easier than valence.
...
- **FEELIN role**: theoretical justification for not treating emotion as a simple label.
...
- **FEELIN role**: methodological justification for using film fMRI as the central affective neuroscience paradigm.
...
- **FEELIN role**: theoretical basis for treating emotion as dynamic, context-dependent representation rather than a static category label.
...
- **FEELIN role**: supports testing stimulus-locked movie fMRI pretraining against resting-state-only transfer, while still requiring emotion downstream validation.
...
- **FEELIN role**: supports the claim that movie-driven brain dynamics carry behaviorally and affectively meaningful signal.
...
- **FEELIN role**: bridge reference for testing how much resting-state representations can predict naturalistic responses.
...
- **FEELIN role**: supports the practical value of movie fMRI for representation learning.
...
- **FEELIN role**: supports explicit video/audio/semantic temporal modeling instead of unimodal fMRI-only targets.
...
- **FEELIN role**: future expansion dataset for naturalistic/physiology-rich pretraining or transfer, not a first two-month priority unless access is easy.
...
- **FEELIN role**: top-conference evidence that stimulus-side affective modeling is moving toward caption/rationale/context-rich supervision.
...
- **FEELIN role**: top-conference precedent for treating video emotion as context/rationale-based representation learning, not only classification.
...
- **FEELIN role**: direct methodological template for cue grounding, hallucination control, and preference-style alignment in brain-tuned affective VLM/LLM track.
...
- **FEELIN role**: diagnostic reference for checking whether stimulus-side affective models invent emotional cues unsupported by audiovisual evidence.
...
- **FEELIN role**: supports using context/rationale embeddings as auxiliary targets, while keeping brain claims grounded in fMRI alignment rather than pure ToM theory.
...
- **FEELIN role**: establishes that "foundation models for affect" is an active AI direction, but also highlights that neuroscience-grounded affective FMs remain underdeveloped.
...
- **FEELIN role**: reference for external affective language models that may support stimulus annotation or text-side affect embeddings.
...
- **FEELIN role**: reference for stimulus-side multimodal affective reasoning, not a replacement for brain modeling.
...
- **FEELIN role**: benchmark-design inspiration for affective evaluation, especially target diversity and intensity prediction.
...
- **FEELIN role**: reference for moving stimulus-side affective supervision beyond label prediction toward reasoning about triggers/context.
...
- **FEELIN role**: stimulus-side affective evaluation reference; useful for defining richer external affective embeddings.
...
- **FEELIN role**: supports the brain-tuned affective LLM/VLM track as a reasoning/representation problem, not just label matching.
...
- **FEELIN role**: potential source of stimulus-side causal/contextual affective targets, kept separate from brain-grounded evaluation.
...
- **FEELIN role**: methodological reference for heterogeneous neural-signal pretraining and emotion downstream transfer.
...
- **FEELIN role**: broad neural-signal FM precedent, especially for multimodal neural data and time-frequency objectives.
...
- **FEELIN role**: useful for architecture and pretraining-objective ideas if physiology or EEG is added later.
...
- **FEELIN role**: conceptual template for an affective brain-score style evaluation of VLM/LLM emotion models.
...
- **FEELIN role**: template for evaluating affective language models against naturalistic emotion fMRI.
...
- **FEELIN role**: direct precedent for brain-tuning external VLM representations with human neural data.
...
- **FEELIN role**: direct methodological precedent for brain-tuned affective LLM/VLM adapters.
...
- **FEELIN role**: supports multi-subject brain-tuning rather than subject-specific overfitting.
...
- **FEELIN role**: supports using modern LLM representations and considering model/data scaling in brain alignment.
...
- **FEELIN role**: analogy for how naturalistic neuroimaging datasets can become model-evaluation infrastructure.
...
- **FEELIN role**: direct cautionary precedent for brain-to-emotion decoding with LLM priors; useful for the brain-tuned affective LLM/VLM track, but not evidence that an fMRI emotion foundation model already exists.
...
- **FEELIN role**: benchmark and data/model reference for TRIBE-style multimodal fMRI response prediction.
...
- **FEELIN role**: high-value expansion dataset if access and preprocessing are feasible.
