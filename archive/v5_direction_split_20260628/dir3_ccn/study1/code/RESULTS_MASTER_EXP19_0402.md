# CCN Results Master Document (Updated Through Experiment 19)

Date: 0402  
Scope: Experiments / scripts 01-19, including 3D, 14D, raw-fMRI, A/V 2D, subject-wise readable-subspace checks, and subject-wise direct decoding.
...
- Subject-wise direct decoding from Brain-JEPA (Exp 19)
...
The single biggest source of confusion in this project is that several spaces are all "models," but they do not play the same methodological role.

Use the following role labels consistently:
...
- `Brain-JEPA` is treated as the main neural space in most model-comparison analyses.
- `raw fMRI` is treated as a complementary neural baseline.
- `V-JEPA2` and `CLIP` are the spaces whose geometry or principal components are compared against neural representations.

So when later sections say "brain-predictable PCs," the meaning is:

- `PCs of V-JEPA2 that are predictable from Brain-JEPA`
- `PCs of CLIP that are predictable from Brain-JEPA`

It does **not** mean "PCs inside Brain-JEPA."

### 2.2 Base Data Objects

All analyses operate on the same `2196` stimulus set.

The base arrays are:

- `Brain-JEPA`: `(5, 2196, 768)`
- `raw fMRI`: `(5, 2196, 450)`
- `V-JEPA2`: `(2196, 1408)`
- `CLIP`: `(2196, 512)`
...
- row = stimulus
- column = feature / PC / parcel / target dimension

So, for example:

- `Brain-JEPA[subject, stimulus, feature]`
- `V-JEPA2[stimulus, feature]`
- `EmotionScore[stimulus, target]`

### 2.3 Aggregation Convention: Mean Embedding First vs Mean RSM

This project uses two different subject-aggregation conventions, and they are different on purpose.

#### A. `Mean embedding first`

Used when the analysis operates on vectors directly.

Definition:

- for each stimulus, average the `5` subject-specific neural embeddings first
- obtain one group-level neural vector per stimulus

Shape change:

- `(5, 2196, 768)` -> mean over subject axis -> `(2196, 768)`

This convention is used for:

- direct decoding from neural embeddings
- predicting model PCs from neural embeddings
- PCA on the neural embedding itself
- most subspace analyses

Why:

- the object of inference is a neural feature vector, not a similarity matrix
- group-level decoding requires a single neural representation per stimulus

#### B. `RSM per subject, then mean RSM`

Used when the analysis operates on geometry itself.

Definition:

- compute a subject-specific `2196 x 2196` RSM first
- then average those RSMs across subjects

Shape change:

- subject-specific embeddings -> `5` subject-specific RSMs -> one mean RSM

This convention is used for:

- geometry-level RSA
- geometry-level CKA
- partial RSA analyses

Why:

- the object of inference is representational geometry itself
- similarity is a nonlinear quantity, so "mean vector then similarity" and "similarity then mean" are not equivalent

#### Practical rule

The cleanest way to state the method is:

- `Embedding-level analyses averaged neural embeddings across subjects first.`
- `Geometry-level analyses averaged subject-specific neural RSMs.`

This is not arbitrary mixing. It is averaging at the level of the object of inference.

### 2.4 Cross-Validation, Thresholds, and Common Modeling Choices

Most predictive analyses share the same modeling template:

- model: `StandardScaler + Ridge(alpha=1.0)`
- score: mean `R²`
- cross-validation: `5-fold`
- negative `R²`: clipped to `0` in many scripts

Important consequences:

- mean `R²` values are directly comparable across many experiments
- some reported averages are slightly optimistic because negative values are floored at `0`
- this project is better understood as representational analysis with predictive components, not as a strict benchmark competition

Common thresholds:

- `brain-predictable PC` threshold: `R² > 0.01`
- `k≈27` is not arbitrary; it comes from repeated `k-sweep` analyses showing near-saturation of alignment and decoding

### 2.5 What Each Core Analysis Type Means

#### `RSM`

For a given space, build a stimulus-by-stimulus similarity matrix:

- rows and columns are stimuli
- cell `(i, j)` measures how similarly the space represents stimulus `i` and stimulus `j`
...
- RSMs let us compare spaces with different feature dimensions by comparing their relational geometry instead of their raw coordinates

#### `RSA`

Compare two RSMs, usually by correlating their upper triangles.

Interpretation:

- high RSA means two spaces organize the stimulus set similarly
- this is a statement about shared relational geometry, not about shared mechanism

#### `CKA`

Compare two representational spaces globally in a scale-insensitive way.

Interpretation:

- high CKA means the spaces share similar overall structure
- again, this is a structural statement, not a claim that the spaces use the same biological computation

#### `Procrustes`

Rotate and scale one low-dimensional space to fit another, then measure the residual mismatch.

Interpretation:

- lower disparity means the two low-dimensional shapes are more similar after optimal linear alignment

#### `Brain-predictable PCs`

For each video-model PC:

- input `X`: neural representation
- target `y`: that PC's value across `2196` stimuli
- score: `5-fold CV R²`

If `R² > 0.01`, that PC is considered readable / predictable from the neural space.

Interpretation:

- these are operationally defined readable axes of video-model space
- this does **not** prove causal readout, only recoverability / alignment

#### `Category vs dimension`

After choosing a feature space or subspace, decode two kinds of affective targets:
...
- mean category `R²`
- mean dimension `R²`
- ratio = category mean / dimension mean

Interpretation:

- ratio > 1: category-leaning
- ratio < 1: dimension-leaning

### 2.6 Methodological Caveats That Matter for Interpretation

These are not side notes; they are central to how the results should be written.

- `Brain-JEPA` is a brain-derived representation, not raw neural activity itself.
- `RSA / CKA / Procrustes` support claims about shared geometry, not shared mechanism.
- `Brain-predictable PC` means predictable from neural representation, not necessarily causally read out by the brain.
- `Category vs dimension` depends on the target ontology:
  - 3D, 14D, and 2D do not ask exactly the same question
- `full-space` direct decoding can become numerically unstable when mean dimension `R²` is near zero.
- `Efficiency` metrics should only be interpreted when the denominator (`all-space R²`) is meaningfully above zero.

### 2.7 Experiment-by-Experiment Method Map

This subsection is meant to make the whole pipeline readable without opening the code.

#### Script 01: Brain-JEPA RSM

Question:

- Do the 5 subjects share a common Brain-JEPA geometry?

Inputs:

- neural input: `Brain-JEPA (5, 2196, 768)`

Method:

- compute subject-specific Brain-JEPA RSMs
- compute cross-subject Spearman similarity among those RSMs
- average subject-specific RSMs to obtain a group neural geometry

Aggregation convention:

- `RSM per subject -> mean RSM`

Primary output:

- cross-subject Brain-JEPA RSM similarity matrix

#### Script 02: Subject-Level CKA

Question:

- For each subject separately, is Brain-JEPA closer to V-JEPA2 or CLIP?

Inputs:

- neural input: subject-specific Brain-JEPA
- model input: V-JEPA2, CLIP

Method:

- compute subject-level CKA between each subject's neural geometry and each model geometry

Aggregation convention:

- subject-level first, then summarize across subjects

Primary output:

- per-subject CKA values and subject mean CKA

#### Scripts 03-04: Cross-Space RSA by Emotion

Question:

- Which emotion structures are strongly reflected in neural and model geometries?
...
- V-JEPA2 geometry
- CLIP geometry
- 34 emotion target vectors

Method:

- build emotion-structure matrices from continuous emotion scores
- correlate each structure with each space's RSM

Aggregation convention:

- geometry-level analysis

Primary output:

- emotion-wise RSA for brain, V-JEPA2, and CLIP

#### Script 05: Brain-JEPA / model k-sweep

Question:

- How many dimensions are needed before shared geometry and decoding nearly saturate?

Inputs:

- group-mean Brain-JEPA embedding
- V-JEPA2 and CLIP embeddings
- affective targets

Method:

- reduce spaces with PCA for multiple `k`
- compare CKA / RSA / decoding as a function of `k`

Aggregation convention:

- `mean embedding first` for embedding-level parts

Primary output:

- `k`-dependent alignment and decoding curves

#### Script 06: Procrustes alignment

Question:

- After low-dimensional reduction, how similarly shaped are neural and model spaces?

Inputs:

- low-dimensional Brain-JEPA
- low-dimensional V-JEPA2
- low-dimensional CLIP

Method:

- align model spaces to neural space with Procrustes
- measure disparity and emotion-weighted residual error

Primary output:

- disparity and per-emotion alignment error

#### Scripts 07-08: Raw fMRI geometry and k-sweep

Question:

- How much of the main story is already visible in raw fMRI?
...
- compute raw-fMRI cross-subject geometry
- compute raw-fMRI-to-model CKA
- run raw-fMRI `k-sweep` decoding

Aggregation convention:

- geometry results: subject-specific RSMs
- decoding results: mean embedding first for group-level summary

Primary output:

- raw cross-subject consistency
- raw-vs-model CKA
- raw k-sweep decoding

#### Scripts 09-10: 2D embedding views and readable-dimension discovery

Question:

- Can the spaces be visualized in 2D, and how many model PCs are readable from neural space?

Inputs:

- Brain-JEPA, raw fMRI, V-JEPA2, CLIP

Method:

- 2D embeddings for qualitative visualization
- for each model PC, predict the PC from neural representation
- define the readable / brain-predictable subspace

Aggregation convention:

- `mean embedding first` for PC predictability

Primary output:

- readable PC masks
- PC predictability curves

#### Experiment 11: Brain-predictable PC × emotion correlation

Question:

- What do the readable PCs actually represent affectively?

Inputs:

- readable PCs from V-JEPA2 / CLIP
- 34 emotion scores
- 3D affective dimensions

Method:

- correlate each readable PC with affective targets
- compare readable PCs vs unreadable PCs

Primary output:

- emotion-correlated readable axes

#### Experiment 12: Category vs dimension from readable model subspace

Question:

- Is the readable model subspace more category-like or dimension-like?

Inputs:

- readable V-JEPA2 PCs
- readable CLIP PCs
- affective targets

Method:

- use selected model PCs as decoding features
- decode categories and dimensions separately
- compare their mean `R²`

Primary output:

- category/dimension balance in readable model subspace

#### Experiment 13: Vision / semantic confound control

Question:

- Is the readable affective signal still present after removing explicit vision and semantic structure?

Inputs:

- neural geometry or readable model subspace
- vision features
- semantic features
- affective targets
...
- partial RSA at the RSM level
- partial `R²` at the target-prediction level

Primary output:

- residual alignment / residual decoding after confound regression

#### Experiment 15: Stability and alpha sensitivity

Question:

- Are readable-PC results stable across subjects, resampling, and ridge alpha?

Inputs:

- Brain-JEPA
- readable model-PC pipeline

Method:

- subject-wise mask recovery
...
#### Experiment 16: Incremental benchmark over vision + semantic baseline

Question:

- Do readable model PCs add anything beyond explicit vision + semantic features?
...
- compare baseline vs baseline+PCs
- report incremental `R²`

Primary output:

- additional variance explained by readable PCs

#### Experiment 17: Arousal / Valence 2D comparison

Question:

- How does the category-vs-dimension story change if the dimension space is reduced to A/V only?

Inputs:

- readable model subspaces
- raw fMRI
- 34 emotion scores + A/V

Method:

- rerun the category-vs-dimension comparison in 2D

Primary output:

- 2D category/dimension balance for model and raw-neural spaces

#### Experiment 18: Subject-wise readable-subspace check

Question:

- If readable model PCs are re-estimated per subject, is the category-vs-dimension orientation preserved?

Inputs:

- subject-specific Brain-JEPA
- V-JEPA2 PCs
- CLIP PCs
- 3D / 14D / 2D targets
...
- for each subject:
  - predict model PCs from that subject's neural embedding
  - threshold readable PCs
  - decode affective targets from the resulting subject-specific readable model subspace

Important note:

- this is a subject-wise readable-model-subspace analysis
- it is **not** direct subject-wise neural decoding

Primary output:

- subject-specific readable masks
- subject-wise category/dimension orientation

#### Experiment 19: Subject-wise direct decoding

Question:

- If we skip model PCs entirely and decode directly from each subject's Brain-JEPA embedding, what category-vs-dimension pattern remains?
...
- decode affective targets directly from:
  - `k=27` PCA-reduced Brain-JEPA
  - `full 768D` Brain-JEPA

Primary output:

- direct subject-wise category/dimension balance
- agreement of subject orientation with the group mean

## 3. Highest-Level Conclusions

Across all analyses through Exp 19, the most stable conclusions are:
...
7. Direct subject-wise decoding from Brain-JEPA does not simply reproduce the readable-model-subspace result: at `k=27`, the group-mean direct-decoding result is dimension-heavy in `3D`, `14D`, and `2D`, and the richest target spaces (`14D`, `2D`) show the clearest dimensional dominance.
...
- [RESULTS_EXP19_0402.md](/pscratch/sd/s/sjmoon/EmoFM/CCN/RESULTS_EXP19_0402.md)
...
The strongest integrated interpretation through Exp 19 is:
...
## 14. Experiment 19: Subject-wise Direct Decoding
...
- whether category-vs-dimension balance is preserved when affective targets are decoded directly from each subject's Brain-JEPA embedding
- whether the low-dimensional `k=27` regime remains superior to direct decoding in the full `768D` Brain-JEPA space
- how this direct neural decoding view compares with the subject-wise readable-model-subspace view from Exp 18

Main numbers (`k=27`):

| Ontology | Mean category `R²` | Mean dimension `R²` | Ratio | Group-mean orientation | Subject agreement |
|---|---:|---:|---:|---|---:|
| 3D | 0.0561 | 0.0597 | 0.9404 | Dimension | 1/5 |
| 14D | 0.0561 | 0.0862 | 0.6511 | Dimension | 5/5 |
| 2D | 0.0561 | 0.0810 | 0.6932 | Dimension | 4/5 |

Interpretation:

- direct subject-wise decoding supports the low-dimensional story: `k=27` is far more interpretable than `full 768D`
- the direct neural decoding result is dimension-heavy overall, especially in `14D` and `2D`
- this does not contradict Exp 18, because Exp 18 was about readable model subspaces, whereas Exp 19 is about direct neural decoding

## 15. Full Record: Experiment 19

This section embeds the full Exp 19 record directly into the master file.
...
## 16. Detailed Records: Scripts 01-10

This section adds the missing scalar / vector / table-level results for the early analysis stack so that this master file can function as a true one-file archive through Exp 19.

As before, very large raw arrays such as full `(2196 x 2196)` RSMs remain in `results/*.npy` and are not pasted verbatim here.

### 16.1 Scripts 01-02: Brain-JEPA RSM and Subject-Level CKA

Source files:
- `results/brain_jepa_rsm_stats.npz`
- `results/subject_cka_results.npz`

#### Brain-JEPA cross-subject RSM consistency

| Subject | Subj1 | Subj2 | Subj3 | Subj4 | Subj5 |
|---|---:|---:|---:|---:|---:|
| Subj1 | 1.0000 | 0.3320 | 0.3185 | 0.2853 | 0.3293 |
| Subj2 | 0.3320 | 1.0000 | 0.3809 | 0.3589 | 0.4122 |
| Subj3 | 0.3185 | 0.3809 | 1.0000 | 0.3270 | 0.3672 |
| Subj4 | 0.2853 | 0.3589 | 0.3270 | 1.0000 | 0.3603 |
| Subj5 | 0.3293 | 0.4122 | 0.3672 | 0.3603 | 1.0000 |

Summary:
- off-diagonal mean = `0.3472`
- off-diagonal std = `0.0342`

#### Per-subject CKA: Brain-JEPA vs V-JEPA2 / CLIP

| Subject | Brain-VJEPA2 CKA | Brain-CLIP CKA | Delta (VJ-CLIP) |
|---|---:|---:|---:|
| Subj1 | 0.054835 | 0.047351 | +0.007484 |
| Subj2 | 0.063292 | 0.060017 | +0.003274 |
| Subj3 | 0.055383 | 0.050774 | +0.004609 |
| Subj4 | 0.045845 | 0.051293 | -0.005448 |
| Subj5 | 0.072584 | 0.060269 | +0.012314 |
| **Mean** | **0.058388** | **0.053941** | **+0.004447** |

Interpretation:
- Brain-JEPA has clear subject-invariant geometry.
- At the subject-level CKA view, V-JEPA2 is modestly ahead of CLIP on average, but not for every subject.

### 16.2 Scripts 03-04: Cross-Space RSA by Emotion

Source file:
- `results/crossspace_rsa_results.npz`
...
Top 5 emotions by RSA in each space:

| Rank | Brain-JEPA | V-JEPA2 | CLIP |
|---:|---|---|---|
| 1 | Uncomfortable `0.0620` | Amusement `0.1803` | Annoyance `0.2200` |
| 2 | Surprise `0.0501` | Annoyance `0.1510` | Interest `0.1510` |
| 3 | Sadness `0.0386` | Romance `0.0984` | Nostalgia `0.1356` |
| 4 | Calmness `0.0370` | Adoration `0.0919` | Amusement `0.1335` |
| 5 | Craving `0.0308` | Envy `0.0730` | Anxiety `0.1299` |

Top 5 alignment / divergence scores:
- Highest alignment (Brain with models): `Uncomfortable 0.0303`, `Empathic pain 0.0268`, `Surprise 0.0187`, `Awkwardness 0.0160`, `Sadness 0.0085`
- Highest divergence (models >> brain): `Amusement 0.2629`, `Annoyance 0.2594`, `Aesthetic appreciation 0.1499`, `Calmness 0.1192`, `Romance 0.1045`

Interpretation:
- Brain RSA peaks for negative/high-salience emotions such as `Uncomfortable`, `Surprise`, and `Sadness`.
- V-JEPA2 and CLIP both show stronger explicit emotion-structure RSA than Brain-JEPA, with CLIP strongest overall.

### 16.3 Scripts 05-06-10: Low-Dimensional Geometry, Procrustes, and Brain-Predictable Dimensions

Source files:
- `results/cka_rsa_vs_k.npz`
- `results/procrustes_results.npz`
- `results/brain_predictable_dims.npz`
- `results/dimensional_emotion_results.npz`
- `results/pc_emotion_correlation.npz`

#### CKA / RSA vs PCA dimension k

| k | CKA Brain-VJ | CKA Brain-CLIP | RSA Brain-VJ | RSA Brain-CLIP |
|---:|---:|---:|---:|---:|
...
Interpretation:
- Most of the gain appears before `k=25~30`.
- This is one of the quantitative bases for the `k≈27` saturation claim.

#### Procrustes alignment at k=27

| Metric | V-JEPA2 | CLIP |
|---|---:|---:|
| Disparity | 0.937986 | 0.938540 |

Emotion-weighted alignment errors:
- Lowest V-JEPA2 error emotions: `Uncomfortable 0.0166`, `Craving 0.0168`, `Surprise 0.0171`, `Calmness 0.0175`, `Sadness 0.0174`
- Lowest CLIP error emotions: `Uncomfortable 0.0163`, `Surprise 0.0164`, `Craving 0.0169`, `Sadness 0.0175`, `Calmness 0.0176`
- Highest V-JEPA2 error emotions: `Guilt 0.0215`, `Annoyance 0.0208`, `Relief 0.0205`
- Highest CLIP error emotions: `Guilt 0.0217`, `Annoyance 0.0208`, `Relief 0.0205`

Interpretation:
- Procrustes says the two model spaces are almost tied once both are reduced to `k=27` and optimally aligned to Brain-JEPA.
- This is why model superiority is safer to discuss as representation-dependent rather than absolute.

#### Brain-predictable PC counts and saturation

| Model | Brain-predictable PCs (`R² > 0.01`) | Saturation by variance-order | Saturation by sorted-R² order |
|---|---:|---:|---:|
| V-JEPA2 | 3 PCs (`PC1, PC2, PC3`) | 3 | 3 |
| CLIP | 5 PCs by saturation metric; 6 PCs by thresholded mask | 5 | 4 |

Thresholded masks actually used in later experiments:
- V-JEPA2: `PC1, PC2, PC3`
- CLIP: `PC1, PC2, PC3, PC5, PC6, PC7`

First 10 PC predictability values:
- V-JEPA2: `0.3728, 0.0748, 0.0878, 0.0003, 0, 0, 0, 0, 0, 0`
- CLIP: `0.2613, 0.1559, 0.1271, 0, 0.1154, 0.0167, 0.0125, 0, 0, 0`

#### A/V/D decoding and PC-A/V/D correlations

| Dimension | Brain-JEPA `R²` | V-JEPA2 `R²` at k=27 | CLIP `R²` at k=27 |
|---|---:|---:|---:|
| Arousal | 0.0000 | 0.0833 | 0.1059 |
| Valence | 0.0652 | 0.1203 | 0.4397 |
| Dominance | 0.0000 | 0.0162 | 0.0834 |

At `k=27`, direct neural decoding of A/V/D is weak and sparse, while model spaces, especially CLIP, capture `Valence` much more strongly.

### 16.4 Scripts 07-08: Raw fMRI Geometry and k-Sweep

Source files:
- `results/raw_rsa_cka_results.npz`
- `results/raw_k_sweep_results.npz`

#### Raw fMRI cross-subject consistency

| Subject | Subj1 | Subj2 | Subj3 | Subj4 | Subj5 |
|---|---:|---:|---:|---:|---:|
| Subj1 | 1.0000 | 0.0886 | 0.0776 | 0.0606 | 0.0612 |
| Subj2 | 0.0886 | 1.0000 | 0.1260 | 0.0852 | 0.0949 |
| Subj3 | 0.0776 | 0.1260 | 1.0000 | 0.0831 | 0.0876 |
| Subj4 | 0.0606 | 0.0852 | 0.0831 | 1.0000 | 0.0660 |
| Subj5 | 0.0612 | 0.0949 | 0.0876 | 0.0660 | 1.0000 |

Summary:
- off-diagonal mean = `0.0831`
- off-diagonal std = `0.0183`

This is much lower than Brain-JEPA (`0.3472`), which quantitatively supports the subject-invariant compression story.

#### Raw fMRI overall CKA with model spaces

| Metric | Value |
|---|---:|
| Mean CKA(raw, V-JEPA2) | 0.1515 |
| Mean CKA(raw, CLIP) | 0.1702 |
| Delta (VJ-CLIP) | -0.0187 |
| p(raw-VJ) | 0.0000 |
| p(raw-CLIP) | 0.0000 |
| p(delta) | 1.0000 |
| 95% CI raw-VJ | [0.1510, 0.1734] |
| 95% CI raw-CLIP | [0.1751, 0.1938] |
| 95% CI delta | [-0.0321, -0.0128] |

Interpretation:
- For raw fMRI, CLIP is actually closer than V-JEPA2 at the global geometry level.
- This is one major reason the paper should avoid a simplistic “V-JEPA2 is always more brain-like” claim.

#### Raw-fMRI k-sweep

| k | Raw decode mean `R²` | V-JEPA2 decode mean `R²` | CLIP decode mean `R²` | Raw-VJ disparity | Raw-CLIP disparity |
|---:|---:|---:|---:|---:|---:|
| 3 | 0.0330 | 0.0550 | 0.0941 | 0.9287 | 0.9365 |
| 5 | 0.0523 | 0.0726 | 0.1366 | 0.9183 | 0.9043 |
| 7 | 0.0683 | 0.0797 | 0.1884 | 0.9190 | 0.8950 |
| 10 | 0.0865 | 0.0955 | 0.2116 | 0.9135 | 0.8941 |
| 15 | 0.0921 | 0.1136 | 0.2361 | 0.9111 | 0.8987 |
| 20 | 0.1017 | 0.1198 | 0.2535 | 0.9135 | 0.8997 |
| 25 | 0.1061 | 0.1291 | 0.2654 | 0.9133 | 0.9022 |
| 27 | 0.1074 | 0.1317 | 0.2694 | 0.9140 | 0.9031 |
| 30 | 0.1088 | 0.1331 | 0.2744 | 0.9147 | 0.9033 |
| 34 | 0.1104 | 0.1399 | 0.2820 | 0.9147 | 0.9036 |
| 40 | 0.1112 | 0.1463 | 0.2836 | 0.9142 | 0.9043 |
| 50 | 0.1137 | 0.1564 | 0.2897 | 0.9137 | 0.9046 |
| 75 | 0.1169 | 0.1677 | 0.2931 | 0.9124 | 0.9054 |
| 100 | 0.1150 | 0.1706 | 0.2907 | 0.9113 | 0.9055 |

Additional summary:
- raw-fMRI k-elbow = `5`
- raw-fMRI decoding plateau estimate = `40`
- Brain-JEPA reference mean decoding at corresponding k values ranges from `0.0156` to `0.0606`

Interpretation:
- Raw fMRI also benefits strongly from low-dimensional compression.
- The `k≈27` regime remains a reasonable shared reference point across analyses.

### 16.5 Scripts 09-10: 2D Embeddings and Visualization Summaries

Source files:
- `results/embedding_2d.npz`
- `results/raw_embedding_2d.npz`

#### Brain-JEPA / model 2D embedding summary

| Quantity | Brain-JEPA-centered 2D embedding | Raw-fMRI-centered 2D embedding |
|---|---:|---:|
| PCA variance explained | 48.7824% | 54.2530% |
| Raw-to-VJEPA2 Procrustes disparity | — | 0.913968 |

Dominant-emotion index pattern (first 10 stimuli):
- Brain-JEPA-centered: `22, 24, 3, 3, 10, 24, 2, 2, 28, 30`
- Raw-fMRI-centered: `22, 24, 3, 3, 10, 24, 2, 2, 28, 30`

Interpretation:
- The low-dimensional overlays are visually useful and broadly consistent with the scalar analyses.
- They are best used as illustration rather than primary evidence.

### 16.6 Practical Early-Stage Summary

What Scripts 01-10 jointly establish is:

1. Brain-JEPA has much stronger subject-invariant geometry than raw fMRI.
2. Brain-model alignment is present for both V-JEPA2 and CLIP, but the winner depends on representation choice and metric.
3. Most shared geometric structure is already visible by roughly `k≈27`.
4. The number of model PCs that are recoverable from Brain-JEPA is very small.
5. This justifies the later move from whole-space alignment to readable-subspace analysis in Exp 11-19.
